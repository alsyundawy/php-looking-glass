...
(function() {
    ...
    themeBtn.addEventListener("click", () => {
        ...
        document.cookie = `theme=${newTheme};path=/;max-age=31536000;samesite=strict${location.protocol === "https:" ? ";secure" : ""}`;
        updateThemeIcon();
    });
    ...
    const fetchIP = async (elementId, apiUrl, fallbackVal) => {
        try {
            ...
            document.getElementById(elementId).textContent = fallbackVal;
        }
    };

    Promise.allSettled([
        fetchIP("clientIPv4", "https://api.ipify.org?format=json", "N/A"),
        fetchIP("clientIPv6", "https://api6.ipify.org?format=json", "N/A")
    ]);

    $(".network-test-form").on("submit", async function(e) {
        e.preventDefault();
        const form = $(this);
        const hostVal = form.find("input[name=host]").val().trim();
        if (!hostVal) {
            alert("Host or IP address required");
            form.find("input[name=host]").focus();
            return;
        }

        let cmdVal = form.find("input[name=cmd]").val();
        const ipVerSelect = form.find("select[name=ipversion]");
        if (ipVerSelect.length && ipVerSelect.val() === "6") {
            cmdVal += "6";
        }

        const outputSec = form.closest(".tab-pane").find(".output-section");
        const outputBox = outputSec.find(".output-box");

        outputSec.removeClass("show");
        outputSec.find(".alert-error").hide();
        outputBox.text("Running command...");
        ...
        try {
            ...
            formData.append("cmd", cmdVal);
            formData.append("csrf", form.find("input[name=csrf]").val());
            ...
            if (isStreamCmd) {
                ...
                for (;;) {
                    ...
                    outputBox.append(document.createTextNode(chunk));
                    $("html,body").animate({ scrollTop: outputSec.offset().top - 80 }, 0);
                }
            } else {
                const text = await response.text();
                outputBox.text(text);
                $("html,body").animate({ scrollTop: outputSec.offset().top - 80 }, 400);
            }
        } catch (err) {
            loader.classList.remove("active");
            const errText = `Error: ${err.message || "Unknown error"}`;
            outputSec.find(".alert-error").text(errText).show();
            outputSec.addClass("show");
        }
    });
...
(function() {
    ...
    function renderWhois(data, container) {
        let html = "";
        if (data.error) {
            container.innerHTML = '<div class="alert-msg alert-error">' + escapeHtml(data.error) + "</div>";
            return;
        }

        if (data.parsed && data.parsed.length) {
            html += '<div class="whois-result-card"><div class="whois-result-header"><i class="fas fa-circle-info"></i> Informasi WHOIS untuk ' + escapeHtml(data.host) + '</div><table class="whois-result-table"><tbody>';
            data.parsed.forEach(function(row) {
                html += '<tr><td><i class="fas ' + escapeHtml(row.icon) + '"></i> ' + escapeHtml(row.key) + '<span class="whois-info-tip"><i class="fas fa-question-circle"></i> ' + escapeHtml(row.info) + "</span></td><td>" + escapeHtml(row.value) + "</td></tr>";
            });
            html += '</tbody></table><button class="whois-raw-toggle" onclick="$(this).next().toggleClass(\'show\');$(this).find(\'i.fa-chevron-down,i.fa-chevron-up\').toggleClass(\'fa-chevron-down fa-chevron-up\')"><i class="fas fa-code"></i> Raw WHOIS Data <i class="fas fa-chevron-down"></i></button><pre class="whois-raw-box">' + escapeHtml(data.raw || "") + "</pre></div>";
        } else {
            html += '<div class="whois-result-card"><div class="whois-result-header"><i class="fas fa-circle-info"></i> Raw WHOIS untuk ' + escapeHtml(data.host) + '</div><pre class="whois-raw-box show">' + escapeHtml(data.raw || "No data returned.") + "</pre></div>";
        }
        container.innerHTML = html;
    }

    function renderDns(data, container) {
        let html = "";
        if (data.error) {
            container.innerHTML = '<div class="alert-msg alert-error">' + escapeHtml(data.error) + "</div>";
            return;
        ...
        types.forEach(function(type) {
            const icon = dnsIcons[type] || "fa-question";
            const list = records[type] || [];
            const count = list.length;
            summary.push({ type: type, count: count });

            html += '<div class="dns-result-card"><div class="dns-type-header"><i class="fas ' + icon + '"></i> ' + type + ' Records <span class="dns-type-badge">' + count + " record" + (count !== 1 ? "s" : "") + "</span></div>";
            if (count > 0) {
                html += '<table class="dns-record-table"><thead><tr><th><i class="fas fa-hashtag"></i> Name</th><th><i class="fas fa-clock"></i> TTL</th><th><i class="fas fa-tag"></i> Type</th><th><i class="fas fa-align-left"></i> Value</th></tr></thead><tbody>';
                list.forEach(function(rec) {
                    html += "<tr><td>" + escapeHtml(rec.name) + "</td><td>" + escapeHtml(rec.ttl) + "</td><td>" + escapeHtml(rec.type) + "</td><td>" + escapeHtml(rec.value) + "</td></tr>";
                });
                html += "</tbody></table>";
            } else {
        ...
        summary.forEach(function(item) {
            html += '<span class="dns-summary-badge ' + (item.count > 0 ? "has-records" : "no-records") + '"><i class="fas ' + (dnsIcons[item.type] || "fa-question") + '"></i> ' + item.type + ": " + item.count + "</span>";
        });
        html += "</div>";
    ...
    $(".ajax-test-form").on("submit", async function(e) {
        e.preventDefault();
        const form = $(this);
        const hostVal = form.find("input[name=host]").val().trim();
        if (!hostVal) {
            alert("Host or domain required");
            form.find("input[name=host]").focus();
            return;
        }

        const cmdVal = form.find("input[name=cmd]").val();
        const csrfToken = form.find("input[name=csrf]").val();
        const pane = form.closest(".tab-pane");
        const outputSec = pane.find(".output-section");
        const resultContainer = pane.find(".ajax-result-container");

        outputSec.removeClass("show");
        outputSec.find(".alert-error").hide();
        resultContainer.html('<div style="text-align:center;padding:2rem;color:var(--text-secondary)"><i class="fas fa-spinner fa-spin" style="font-size:1.5rem;margin-bottom:.5rem;display:block"></i>Memproses ' + escapeHtml(cmdVal === "whois" ? "WHOIS" : "DNS") + " lookup untuk <strong>" + escapeHtml(hostVal) + "</strong>...</div>");
        outputSec.addClass("show");
        loader.classList.add("active");
        ...
        try {
            ...
            if (!response.ok) {
                outputSec.find(".alert-error").text((await response.text()) || "Error occurred").show();
            ...
            if (cmdVal === "whois") {
                renderWhois(data, resultContainer[0]);
            } else if (cmdVal === "dnslookup") {
                renderDns(data, resultContainer[0]);
            }

            $("html,body").animate({ scrollTop: outputSec.offset().top - 80 }, 400);
        } catch (err) {
            loader.classList.remove("active");
            outputSec.find(".alert-error").text("Error: " + (err.message || "Unknown error")).show();
            resultContainer.html("");
        }
    });
})();
